...
class Model(nn.Module):
    ...
    def forward(self, U, qmask, umask=None, att2=False):
        ...
        if att2:
            att_emotions = []
            for t in emotions:
                att_emotions.append(self.matchatt(
                    emotions, t, mask=umask)[0].unsqueeze(0))
            att_emotions = torch.cat(att_emotions, dim=0)
...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, output_size, filters, kernel_sizes, dropout):
        ...
        self.convs = nn.ModuleList([nn.Conv1d(
            in_channels=embedding_dim, out_channels=filters, kernel_size=K) for K in kernel_sizes])
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, umask):
        ...
        # (num_utt * batch, 150) -> (num_utt * batch, 100)
        ...
        mask = umask.unsqueeze(-1).type(FloatTensor)
        # (batch, num_utt, 1) -> (num_utt, batch, 1)
        mask = mask.transpose(0, 1)
        # (num_utt, batch, 1) -> (num_utt, batch, 100)
        mask = mask.repeat(1, 1, self.feature_dim)
        # (num_utt, batch, 100) -> (num_utt, batch, 100)
...
class DailyDialogueModel(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, D_h,
        ...
        self.cnn_feat_extractor = CNNFeatureExtractor(
            vocab_size, embedding_dim, cnn_output_size, cnn_filters, cnn_kernel_sizes, cnn_dropout)
